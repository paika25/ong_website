...
async function loadDossiers() {
  ...
  try {
    ...
    const data = await $fetch<DossierCard[]>('/api/back-office/dossiers', {
...
async function openDossier(card: DossierCard) {
  ...
  try {
    ...
    selectedDossier.value = await $fetch(`/api/back-office/dossiers/${card.id}`, {
      headers: { Authorization: `Bearer ${session?.access_token}` }
    })
...
async function callAction(ongId: string, action: string, body: Record<string, any> = {}) {
  const supabase = useSupabase()
  const { data: { session } } = await supabase!.auth.getSession()
  return $fetch(`/api/back-office/dossiers/${ongId}/${action}`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${session?.access_token}` },
    body,
  })
