  ...
  <div class="flex flex-col h-full">
    ...
    <div v-if="!hideHeader" class="flex items-center justify-between px-4 py-2.5 border-b border-border shrink-0">
      <div class="flex items-center gap-2">
        <Icon name="i-heroicons-shield-check" class="w-4 h-4 text-primary" />
        <span class="text-sm font-semibold">Administration Paika</span>
        <span
          v-if="unreadCount > 0"
    ...
    <div ref="scrollEl" class="flex-1 overflow-y-auto p-4 space-y-3 min-h-0">
      ...
      <template v-else>
        ...
        >
          ...
          >
            {{ msg.sender_role === 'back_office' ? 'BO' : 'BP' }}
          </div>

          <div class="flex flex-col gap-0.5 max-w-[75%]" :class="isMine(msg) ? 'items-end' : ''">
            <span class="text-xs text-muted-foreground">
              {{ msg.sender_role === 'back_office' ? 'Administration' : 'Bailleur' }}
...
import {
  fetchPartnerAdminMessages,
  postPartnerAdminMessage,
} from '~/features/messaging/services/partner-admin-messagerie.service'
import type { PartnerAdminMessage } from '~/features/messaging/services/partner-admin-messagerie.service'

const props = defineProps<{
  partnerId: string
  viewerRole: 'back_office' | 'partner'
  hideHeader?: boolean
}>()
...
const messages = ref<PartnerAdminMessage[]>([])
const draft = ref('')
...
function isMine(msg: PartnerAdminMessage) {
...
async function load() {
  loading.value = true
  try {
    messages.value = await fetchPartnerAdminMessages(props.partnerId)
    scrollToBottom()
    await markRead()
  } catch {
...
async function send() {
  ...
  sending.value = true
  try {
    const msg = await postPartnerAdminMessage(props.partnerId, content)
...
function subscribeRealtime() {
  ...
  realtimeChannel = supabase
    .channel(`partner-admin-messages-${props.partnerId}`)
    .on(
      'postgres_changes',
      { event: 'INSERT', schema: 'public', table: 'partner_admin_messages', filter: `partner_id=eq.${props.partnerId}` },
      (payload: any) => {
        const exists = messages.value.some(m => m.id === payload.new.id)
        if (!exists) {
          messages.value.push(payload.new as PartnerAdminMessage)
          scrollToBottom()
